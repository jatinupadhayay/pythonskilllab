...
def export_timetable():
    ...
    if file_path:
        try:
            ...
            # Create a table with 1 header row and as many columns as there are fields in each row
            table = doc.add_table(rows=1, cols=len(generated_timetable[0]))

            hdr_cells = table.rows[0].cells
            for i, col_name in enumerate(("Date", "Time", "Year", "Subject", "Room Number", "Building", "Faculty")):
